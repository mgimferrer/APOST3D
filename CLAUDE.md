...
**Independently verified against the original pre-migration outputs (July 2026):**
`../REFERENCE-OUTPUTS/compiler-testset/*.apost1`/`.apost2` (outside this repo)
are the original ifort/PGO outputs, predating the gfortran migration —
generated on different hardware (48-core, x86-64) with `OMP_NUM_THREADS`
unset. Diffing them against `tests/reference/*.apost` (ignoring `Elapsed
time`/thread-count lines) gives: **H2O-T-B3LYP, CH3F, FeCO2-PBEPBE, FeO4-2 —
byte-for-byte identical** on every numerical line. **C2H6-B3LYP** — every
numerical result line identical; the only diffs are cosmetic (list-directed
`E(SCF/DFT)=` print-precision formatting differs between ifort/gfortran, one
diagnostic message dropped the word "atoms" at some point in the code's
history, and thread-count banner lines differ because the original was run
multi-threaded on a 48-core machine vs. `OMP_NUM_THREADS=1` here). This is a
stronger check than comparing against `tests/reference/` alone (which was
itself generated by the gfortran build under test) — it confirms the
gfortran/Apple-Silicon build reproduces the *original* ifort/PGO results,
not just its own prior runs.

**`manifest.json` "ref" values confirmed sourced from the original outputs
(July 2026):** every check's regex `pattern` was run directly against
`../REFERENCE-OUTPUTS/compiler-testset/*.apost1` (not `tests/reference/`)
and compared to the `ref` value stored in `manifest.json`. All 39 checks
across all 5 tests (H2O-T-B3LYP, C2H6-B3LYP, CH3F, FeCO2-PBEPBE, FeO4-2)
extract a value from the original ifort/PGO output that matches the
manifest's `ref` exactly (Δ 0.0e+00) — i.e. the reference values the test
suite checks against are not just self-consistent with the gfortran build,
they are traceable to the original pre-migration ground truth. See
`tests/coverage.py`-adjacent tooling note: there's no automated script for
this cross-check yet — it was done manually; worth scripting if
`REFERENCE-OUTPUTS` stays available long-term (see Planned Work).
...
- [x] **`manifest.json` ref values verified against original outputs (July 2026)**: Every check's regex `pattern` in `tests/manifest.json` was run directly against `../REFERENCE-OUTPUTS/compiler-testset/*.apost1` (the original pre-migration ifort/PGO outputs) and compared to the stored `ref` value. All 39 checks across all 5 tests match exactly (Δ 0.0e+00) — the manifest's reference values are traceable to the original ground truth, not just self-consistent with the gfortran build that generated `tests/reference/`. Done manually this time; see next item.
- [ ] **Script the `REFERENCE-OUTPUTS` cross-check**: Turn the manual verification above into a small script (e.g. `tests/verify_against_original.py`) that runs every manifest check's pattern against `../REFERENCE-OUTPUTS/compiler-testset/*.apost1` and flags any drift. Useful as a one-off run now, but would matter more if `make update-ref` is ever run carelessly (it regenerates `ref` values from whatever the current build produces, with no automatic cross-check against the original ifort/PGO ground truth) — this catches manifest values silently drifting away from the original reference over time. `REFERENCE-OUTPUTS` lives outside this repo (`../REFERENCE-OUTPUTS`), so this is a local/manual tool, not something CI can run.
