...
15. **dyld / code-signature failure at runtime on macOS (fixed in Phase 0, July 2026)**: A gfortran-compiled binary that links successfully can still fail to *launch* on macOS (especially Apple Silicon) with `dyld[...]: Library not loaded: /usr/lib/libSystem.B.dylib ... (no such file, no dyld cache)`. This looks like a missing system library but is actually a missing/invalid ad-hoc code signature (or a stray `com.apple.quarantine` extended attribute) — the kernel refuses to let an untrusted binary map the shared dyld cache at all. `make_compile.sh` already ad-hoc-signs the three binaries after building, but previously treated a `codesign` failure as a silent non-fatal warning, and nothing actually *executed* the binaries to confirm they could launch — so the failure only surfaced later, independently, for every single test case in the suite (same root cause, same error, reported N times). Fixed: `make_compile.sh` now clears extended attributes (`xattr -c`) before signing, surfaces real `codesign` errors instead of swallowing them, and — new — actually launches all three binaries with `< /dev/null` as a build-time smoke test, printing a clear diagnostic (`xattr -cr` + `codesign --force --sign -` on each binary) if any of them fail to start. Verified the smoke-test detection logic directly against built binaries; the macOS-specific codesign path itself can't be exercised in this (Linux) environment.
