...
C reads .log file as argument and prints on screen (append to .fchk file)
      ...
      character*80 line,name0,cc1
           
      call getarg(1,name0)    
      open(unit=1,file=name0)

c     To see if there is pseudopotential
...
c     Extracting the energies from the .log file
      ...
      do while(index(line,"Normal term").eq.0)
        read(1,'(a80)') line
        if(index(line," KE=").eq.1) then
          read(line(5:23),'(f19.8)') ykin
          read(line(28:46),'(f19.8)') ype
          read(line(51:69),*) yee
        end if
      end do

      cc1="Kinetic Energy "
      write(*,'(A43,A6,ES22.15)') cc1,"R     ",ykin
      cc1="Electron-Nuclei Energy "
      write(*,'(A43,A6,ES22.15)') cc1,"R     ",ype
      cc1="Electron-Electron Energy "
      write(*,'(A43,A6,ES22.15)') cc1,"R     ",yee 
...
c     Kinetic Energy
      ...
      call readXmatrix(xkem,bf,"Kinetic ")
...
c     Potential Energy
      ...
      call readXmatrix(xpem,bf,"Potentia")
